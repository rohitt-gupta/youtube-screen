...
const SideBar = () => {
  ...
  return (
    <>
      ...
      <aside className={` w-56 lg:sticky absolute top-0 overflow-y-auto scrollbar-hidden pb-4 flex-col gap-2 lg:flex ${isLargeOpen ? "lg:flex" : "lg:hidden"} ${isSmallOpen ? "flex z-[999] bg-white max-h-screen" : "hidden"}`}>
        ...
        <LargeSidebarSection title="Subscriptions" visibleItemCount={5}>
          {subscriptions.map(sub => (
            <LargeSidebarItem key={sub.id} IconOrImgUrl={sub.imgUrl} title={sub.channelName} url={`/@subscription${sub.id}`} />
          ))}
        </LargeSidebarSection>
        <hr />
...
const LargeSidebarItem = ({ IconOrImgUrl, title, url, isActive }: LargeSidebarItemProps) => {
  return (
    <a href={url} className={twMerge(buttonStyles({ variant: "ghost" }), `w-full flex items-center rounded-lg gap-4 p-3 ${isActive ? 'font-bold bg-neutral-100 hover:bg-secondary gap-4 p-3' : undefined}`)}>
      {typeof IconOrImgUrl === 'string' ? <img src={IconOrImgUrl} alt={title} className="w-6 rounded-full" /> : <IconOrImgUrl className="w-6 h-6" />}
